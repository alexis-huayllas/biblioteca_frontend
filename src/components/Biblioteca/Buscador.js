...
function Buscador(){
    ...
    useEffect(()=>{
        const ListData = async () => {
            try {
              const response = await axios.get(endPointsbiblioteca.obtenerpaginabibliotecaordenada.url+'titulo/data'); // Reemplaza con la URL de tu API
    ...
    return (<>
        ...
        <div style={{height:'100%'}}>
            ...
            <>{listado.length>0?listado.map((dato,m)=><>{/*<div style={{display:'flex',flexDirection:'row',marginBottom:'10px',padding:'10px',border:'1px solid #ccc',borderRadius:'5px'}}>
